  ...
  begin
    bLogin := GeralDMfrm.funcoes.ValidaLogin( edtUsuario.Text, edtSenha.Text );
      if bLogin then
        ...
        MessageBox( Handle, pwidechar( 'Não foi encontrado o login! Por favor faça seu cadastro. ' ), 'Atenção', MB_OK + MB_ICONWARNING );
  end
  else
  MessageBox( Handle, pwidechar( 'Por favor, preencha todos os campos acima! ' ), 'Atenção', MB_OK + MB_ICONWARNING );
end;
...
begin
  ...
  MessageBox( Handle, pwidechar( 'Por favor preencha todos os campos antes de salvar!' ), 'Atenção', MB_OK + MB_ICONWARNING );
